...
class BalzacSocial extends WP_Widget{
	...
	public function __construct(){
		parent::__construct(
			'BalzacSocial',
			__('Balzac - Social', 'balzac'),
			array('description'=>__('Display links to your social profiles.', 'balzac'),)
		);
	}
	...
	public function form($instance){
		...
		foreach ($fields as $count=>$field){
			...
			<div>
				<a href="#" class="balzacsocial-toggle-link">
				</a>
				<h4>
					<?php _e('More networks', 'balzac'); ?>
				</h4>
			</div>
			<div class="balzacsocial-toggle <?php echo $open; ?>">
			<?php endif; ?>
			
			<?php $value = (isset($instance[$field])) ? $instance[$field] : ''; ?>
			<p>
				<label for="<?php echo $this->get_field_id($field); ?>">
					<?php echo ($field == 'title' ? __('Title','balzac') : ucfirst(str_replace("-"," ",$field)) ) . ':'; ?>
				</label> 
 ...
 * @return void
 */
if (!function_exists('balzac_widgets_init')){
	function balzac_widgets_init(){
		register_widget('BalzacSocial');
	}
}
add_action('widgets_init', 'balzac_widgets_init');